...
RiotRequest.prototype.generateQueue = function generateQueue(region) {
  ...
  function queueWorker(task, done) {
    ...
    var fetcher = function getFromRiot(cb) {
      ...
      var now = new Date();
      if(now.getTime() - requestQueue.lastNetworkCall.getTime() > 10500) {
    ...
    async.waterfall([
      function getFromCache(cb) {
        if(self.requestQueues[region].lastEndpoint !== task.endpoint) {
          self.cache.get(region, task.endpoint, rarity.slice(2, cb));
        }
        else {
          // No call has been made since we queued this request, we can safely assume the cache hasn't changed.
          cb(null, null);
        }
      ...
      function actOnCache(cachedData, cb) {
        if(cachedData) {
          secondaryLog("Read from cache " + region + ":" + task.endpoint + " after initial cache-miss");
...
RiotRequest.prototype.request = function request(region, endpoint, cacheStrategy, done) {
  ...
  async.waterfall([
    ...
    function actOnCache(cachedData, cb) {
      if(cachedData) {
        requestQueue.log("Read from cache " + region + ":" + endpoint);
      ...
      requestQueue.lastEndpoint = endpoint;
    },
