...
RiotRequest.prototype.generateQueue = function generateQueue(region, method) {
  ...
  function queueWorker(task, done) {
    ...
    var fetcher = function getFromRiot(cb) {
      ...
      var now = new Date();
      if(now.getTime() - bucketStartedAt.getTime() > timeBeforeReset + 50) {
        ...
        console.log("Resetting queue concurrency to " + defaultConcurrency);
      }
      ...
      superagent
        ...
        .end(function(err, res) {
          if(bucketStartedAt === now) {
            ...
            console.log("Updated bucket start date");
          ...
          if(res && res.headers && res.headers['x-app-rate-limit-count'] && res.headers['x-method-rate-limit']) {
            ...
            timeBeforeReset = Math.min.apply(Math, limit.map(l => l[1])) * 1000;
            ...
            availableConcurrency = Math.max(1, availableConcurrency - requestQueue.running());

            console.log(callsLeft, "AV:", availableConcurrency, "CUR:", requestQueue.concurrency, "RUN:", requestQueue.running());
          ...
          if(err && err.status === 429) {
            ...
            console.log("Rate limited, will retry in " + retryAfter + " (pending requests: " + (requestQueue.length() + 1) + ")");
