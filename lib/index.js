      ...
      superagent
        ...
        .end(function(err, res) {
          if(res && res.headers && res.headers['x-app-rate-limit-count'] && res.headers['x-method-rate-limit']) {
            ...
            timeBeforeReset = Math.min.apply(Math, limit.map(l => l[1]));
            console.log("CONC" + defaultConcurrency + " and T:" + timeBeforeReset);
          ...
          if(err && err.status === 429) {
            ...
            secondaryLog("Rate limited, will retry in " + retryAfter + " (pending requests: " + (requestQueue.length() + 1) + "), current " + task.endpoint);
